...
export function isContains({ query, target }: Params): boolean {
  // ひらがな・カタカナ・半角・全角をすべてひらがなに変換する
  const normalizedQuery = query.normalize('NFKC').replace(/[ぁ-んァ-ンｧ-ﾝ]/g, (char) => {
    return String.fromCharCode(char.charCodeAt(0) - 0x60);
  });
  const normalizedTarget = target.normalize('NFKC').replace(/[ぁ-んァ-ンｧ-ﾝ]/g, (char) => {
    return String.fromCharCode(char.charCodeAt(0) - 0x60);
  });
